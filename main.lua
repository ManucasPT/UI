...
game:GetService("RunService").RenderStepped:Connect(function()
    ...
    do
        ...
        local offset = (currentCFrame.LookVector * -0.5) + (currentCFrame.RightVector * 0) + (currentCFrame.UpVector * 0)
        camera.CFrame = currentCFrame + offset
    end
    -- Crosshair
    do
        Crosshair.Position = Vector2.new(game:GetService("UserInputService"):GetMouseLocation().X, game:GetService("UserInputService"):GetMouseLocation().Y);
    end
    -- Speedhack
    do 
